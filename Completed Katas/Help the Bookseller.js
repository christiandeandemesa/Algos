...
function stockList(listOfArt, listOfCat) {
    ...
    // Removes the edge case if listOfArt or listOfCat is an empty array.
    return (!listOfArt.length || !listOfCat.length) ? "" : listOfCat.map(cat => {

        /*
        needs is going to be every element in the listOfArt array that starts with the given letter from the listOfCat array. If listOfArt is 
        ["ABAR 200", "CDXE 500", "BKWR 250", "BTSQ 890", "DRTY 600"] and listOfCat is ["A", "B"], then  the second cat is 'B' and needs is 
        'BKWR 250,BTSQ 890'.
        */
